...
export const NavMenu = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  return (
    <div className={`fixed inset-0 bg-black text-white top-4/12  py-6 transition-transform duration-500 z-40 lg:hidden ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
     

      <ul className="flex flex-col gap-4 bg-primary p-4  text-base font-medium mt-4">
        {navItems.map((item) =>
          item.children ? (
